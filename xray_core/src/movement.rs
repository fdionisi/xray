...
pub fn left(buffer: &LocalBuffer, mut point: Point) -> Point {
    if point.column > 0 {
        point.column -= 1;
    } else if point.row > 0 {
        point.row -= 1;
...
pub fn right(buffer: &LocalBuffer, mut point: Point) -> Point {
...
pub fn beginning_of_word(buffer: &LocalBuffer, mut point: Point) -> Point {
...
pub fn end_of_word(buffer: &LocalBuffer, mut point: Point) -> Point {
    // TODO: remove this once the iterator returns char instances.
...
pub fn end_of_line(buffer: &LocalBuffer, mut point: Point) -> Point {
    point.column = buffer.len_for_row(point.row);
    point
}
